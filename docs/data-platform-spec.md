...
|--------|--------|-----------|---------------------|
| **아이템스카우트** | 이커머스(네이버 스마트스토어) | 키워드 분석, 경쟁 상품 분석, 트렌드 예측 | RaceScout(경주 발굴), Odds Radar(배당 분석) |
...
| **이변 가능성** | `upset_probability` | 과거 유사 경주 이변 발생률 | 예측 불확실성 |
...
| 메트릭 | 영문명 | 설명 |
|--------|--------|------|
| **적중률** | `hit_rate` | 조건 충족 후 1착 비율 |
| **복승 적중률** | `place_hit_rate` | 조건 충족 후 1-2착 비율 |
...
| **ML 기반 승률 예측 모델 상세 설계** | 데이터 축적 우선 필요 | Phase 4 이후 검토 |
